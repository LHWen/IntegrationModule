...
- (void)addPhotoToView:(UIImage *)image addImagePath:(NSString *)imagePath {
    ...
    [_imageArray addObject:[self zipNSDataWithImage:image]];
...
- (UIImage *)zipNSDataWithImage:(UIImage *)sourceImage{
    ...
    if (data.length > 40 * 1024 * 1024) { // 1M以及以上
        data = UIImageJPEGRepresentation(newImage, 0.05);
    } else if (data.length > 20 * 1024 * 1024) {
        data = UIImageJPEGRepresentation(newImage, 0.1);
    } else if (data.length > 10 * 1024 * 1024) {
        data = UIImageJPEGRepresentation(newImage, 0.2);
    } else if (data.length > 4 * 1024 * 1024) {
