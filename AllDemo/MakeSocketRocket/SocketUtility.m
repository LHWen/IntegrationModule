...
@interface SocketUtility ()<SRWebSocketDelegate> {
    int _index;
    NSTimer * heartBeat;
...
+ (SocketUtility *)instance{
    static SocketUtility *Instance = nil;
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        Instance = [[SocketUtility alloc] init];
    });
    return Instance;
}
...
- (void)sentheart {
    // 发送心跳 和 后台可以约定发送什么内容  一般可以调用ping  我这里根据后台的要求 发送了data给他
    [self sendData:@"heart"];
}
